...
class ApiClient {
  ...
  Future<Map<String, dynamic>> postMultipart(
    ...
    try {
      ...
      for (var entry in files.entries) {
        ...
        final contentType = _toSupportedImageMediaType(detectedMimeType);

        if (kDebugMode) {
          print('📎 Adding file: $fieldName = $fileName (${file.path})');
          print('📎 Detected mime type: ${detectedMimeType ?? "unknown"}');
        }
  ...
  /// Allow only API-supported image MIME types when sending multipart images.
  MediaType? _toSupportedImageMediaType(String? mimeType) {
