...
class AuthService {
  ...
  /// Build a stable device identifier for login device binding.
  Future<String> _getDeviceId() async {
    try {
      ...
      if (kIsWeb) {
        ...
        return 'web:${webInfo.vendor ?? 'unknown'}:${webInfo.userAgent ?? 'ua'}';
      }

      if (Platform.isAndroid) {
        final androidInfo = await deviceInfo.androidInfo;
        final raw =
            '${androidInfo.brand}:${androidInfo.model}:${androidInfo.id}:${androidInfo.device}';
        return 'android:${sha256.convert(utf8.encode(raw)).toString()}';
      }
      ...
      if (Platform.isIOS) {
        ...
        return 'ios:${sha256.convert(utf8.encode(raw)).toString()}';
      }

      final raw = '${Platform.operatingSystem}:${Platform.localHostname}';
      return 'other:${sha256.convert(utf8.encode(raw)).toString()}';
    } catch (_) {
      // Last-resort fallback to avoid blocking login.
      final fallback =
          '${DateTime.now().millisecondsSinceEpoch}:${Random().nextInt(1 << 20)}';
      return 'fallback:${sha256.convert(utf8.encode(fallback)).toString()}';
    }
  }
  ...
  Future<AuthResponse> login({
    ...
    try {
      ...
      if (kDebugMode) {
        print('🌐 Runtime Login Endpoint: ${ApiEndpoints.login}');
      }
      ...
      if (e is ApiException) {
        ...
        // Surface backend response message directly instead of static fallback.
  ...
  Future<AuthResponse> register({
    ...
    required String idBackImagePath,
  }) async {
    try {
      final fields = <String, String>{
        ...
        'last_name': lastName,
        // Backward compatibility with legacy register schema.
      ...
      if (parentPhone != null && parentPhone.isNotEmpty) {
        ...
        fields['parent_phone'] = parentPhone;
      }
      ...
      if (role == 'student' && studentType != null) {
        ...
        if (studentType == 'in_person') {
          ...
          mappedStudentType = 'online';
        }
        fields['student_type'] = mappedStudentType;
      }

      if (selectedCategoryId != null && selectedCategoryId.isNotEmpty) {
        fields['selected_category_id'] = selectedCategoryId;
      }
      if (selectedSubcategoryId != null && selectedSubcategoryId.isNotEmpty) {
        fields['selected_subcategory_id'] = selectedSubcategoryId;
      }
      if (categoryPath != null && categoryPath.isNotEmpty) {
        fields['category_path'] = categoryPath;
      }
      ...
      final response = await ApiClient.instance.postMultipart(
        ...
        fields: fields,
        files: {
          'id_front_image': File(idFrontImagePath),
          'id_back_image': File(idBackImagePath),
        },
        requireAuth: false,
      );
      ...
      if (response['success'] == true) {
        ...
        final status = (user['status'] ?? data['status'])?.toString();
        ...
        if (status?.toLowerCase() == 'pending') {
          ...
          // Return response so UI can route to pending approval screen.
  ...
  Future<void> logout() async {
    try {
      ...
      await ApiClient.instance.post(ApiEndpoints.logout, requireAuth: true);
    } catch (e) {
  ...
  Future<AuthResponse> signInWithGoogle() async {
    try {
      ...
      if (e.toString().contains('PlatformException') ||
        ...
        throw Exception(
          'فشل تسجيل الدخول عبر Google. يرجى التحقق من:\n'
          '- اتصال الإنترنت\n'
