...
class CourseAccessLockedException implements Exception {
  final String message;
  CourseAccessLockedException(this.message);
...
class CoursesService {
  ...
  Future<Map<String, dynamic>> getLessonDetails(
    ...
    try {
      ...
      if (e.statusCode == 403 && e.message.contains('هذا الكورس مقفل')) {
        throw CourseAccessLockedException(e.message);
      }
      rethrow;
    } catch (e) {
      rethrow;
  ...
  Future<List<Map<String, dynamic>>> getCategories(
      {bool useAdmin = false, bool requireAuth = true}) async {
    final url =
  ...
  /// Get subcategories (supports category root or parent children).
  Future<List<Map<String, dynamic>>> getSubcategories({
    ...
    try {
      ...
      if (parentId != null && parentId.trim().isNotEmpty) {
        queryParams['parent_id'] = parentId.trim();
      }

      final queryString = queryParams.isEmpty
          ? ''
          : '?${Uri(queryParameters: queryParams).query}';
      final url = '${ApiEndpoints.subcategories}$queryString';
