...
class _SplashScreenState extends State<SplashScreen>
  ...
  Future<void> _checkFirstLaunch() async {
    ...
    try {
      ...
      final status = (meData['status'] ?? '').toString().toLowerCase();
      final accountStatus =
          (meData['account_status'] ?? meData['accountStatus'] ?? '')
              .toString()
              .toLowerCase();
      final rejectionReason = (meData['account_rejection_reason'] ??
              meData['accountRejectionReason'] ??
              '')
          .toString();
      ...
      if (status == 'rejected' || accountStatus == 'rejected') {
        context.go(
          ...
          extra: rejectionReason.isEmpty ? null : rejectionReason,
        );
        return;
      }
    } catch (_) {}
